...
public class ArchUnitTestCode {
        ...
        public void arch() {
                ...
                eventPackageRule.check(javaClasses);
                cycleCheck.check(javaClasses);
                accountPackageRule.check(javaClasses);

        }

        @Test
        @DisplayName("순환 참조 슬라이스 테스트")
        void slice(){
                JavaClasses javaClasses = new ClassFileImporter().importPackages("com.study.inf.modules");
                ArchRule cycleCheck = slices().matching("com.study.inf.modules.(*)..").should().beFreeOfCycles();
                cycleCheck.check(javaClasses);
        }
